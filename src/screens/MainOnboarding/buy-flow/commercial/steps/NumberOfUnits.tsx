...
import { searchCommercialNumberOfUnits, searchCommercialData, searchSelector, searchResultCountSelector } from 'src/redux/selectors/search';
...
const NumberOfUnitsScreen = (props: Props) => {
  const { onNext, navigation } = props;
  const [options, setOptions] = useState<NumberOfUnits[]>([]);
  const stateNumberOfUnits = useSelector(searchCommercialNumberOfUnits);
  ...
  return (
    <Page keyboardAvoidingEnabled={false}>
      <Container style={styles.contentContainer}>
        <StepperTitle title='Number of units' />
