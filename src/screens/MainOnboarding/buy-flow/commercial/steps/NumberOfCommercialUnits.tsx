...
import { searchCommercialData, searchCommercialNumberOfCommercialUnits, searchResultCountSelector } from 'src/redux/selectors/search';
...
const CommercialNumberOfCommercialUnits = (props: Props) => {
  const { onNext, navigation } = props;
  const [options, setOptions] = useState<NumberOfUnits[]>([]);
  const stateNumberOfUnits = useSelector(searchCommercialNumberOfCommercialUnits);
  ...
  const setOption = (option: NumberOfUnits) => {
    ...
    dispatch(updateSearchFilter({
      commercialData: {
        ...stateCommercialData,
        numberOfCommercialUnits: copiedOptions,
      }
    }));
  }
  ...
  return (
    <Page keyboardAvoidingEnabled={false}>
      <Container style={styles.contentContainer}>
        <StepperTitle title='Number of commercial units' />
...
export default CommercialNumberOfCommercialUnits;
