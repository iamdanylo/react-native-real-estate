...
import { ScrollView, StyleSheet, View, Text, Alert, KeyboardAvoidingView, TouchableOpacity } from 'react-native';
...
import { currentPropertyId, currentPropertyCommercialDetails, currentPropertySize, currentPropertySquare } from 'src/redux/selectors/currentProperty';
import { saveProperty } from 'src/redux/actions/currentProperty';
import { AreaLocation, MeasureSquare, UIArea } from 'src/types';
import { convertSquareDetailsToUI, convertUISquareDetails, createRow, generateSquareRows, validateRows } from 'src/utils/squareScreens';
...
const CommercialSquare = (props: Props) => {
  ...
  useEffect(() => {
    if (statePropertySquare) {
      ...
      if (!rows.length && statePropertyDetails?.commercialUnits) {
        generateRows(statePropertyDetails?.commercialUnits, 'unit');
      }

      if (!rows.length && statePropertyDetails?.commercialResidentialUnits) {
        generateRows(statePropertyDetails?.commercialResidentialUnits, 'residential unit');
      }

      if (!rows.length && statePropertyDetails?.numberOfCommercialUnits) {
        generateRows(statePropertyDetails?.numberOfCommercialUnits, 'commercial unit');
      }
    }
...
export default CommercialSquare;
