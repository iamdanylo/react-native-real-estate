...
import { OPTIONS } from 'src/constants/search/LandTypeOptions';
...
const LandTypeScreen = (props: Props) => {
  const { onNext, navigation } = props;
  const [chosenTypes, setChosenTypes] = useState<PropertyDetailedType[]>([]);
  const stateLandType = useSelector(searchDetailedType);
  ...
  const stateType = isSearchFlow ? stateLandType : stateCurrentProperty?.detailedType;
